...
export const TransferButton = () => {
    ...
    const { fromAmount: amount, fromToken: token, fromBalance: balance, tokenLimits, allowed } = useContext(BridgeContext);
    ...
    const onClick = () => {
        ...
        if (!ethersProvider) {
            ...
            setMessage(`Please specify amount more than ${formatValue(tokenLimits.minPerTx, token.decimals)}`);
        } else if (window.BigInt(amount) >= window.BigInt(tokenLimits.maxPerTx)) {
            setMessage(`Please specify amount less than ${formatValue(tokenLimits.maxPerTx, token.decimals)}`);
    ...
    return (
        ...
        >
            {isOpen && message && <ErrorModal message={message} isOpen={isOpen} onClose={onClose} />}
            {isOpen && !message && <ConfirmTransferModal isOpen={isOpen} onClose={onClose} />}
